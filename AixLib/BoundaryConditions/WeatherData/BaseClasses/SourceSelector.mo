...
equation
  if datSou == AixLib.BoundaryConditions.Types.DataSource.Parameter then
    y = p;
  end if;
  connect(uCon, y);
  connect(uFil, y);
    ...
    Icon(graphics={
      Rectangle(
        ...
        Line(points={{-38,80},{6,2}},
          color={0,0,127},
          visible=datSou == AixLib.BoundaryConditions.Types.DataSource.File,
          thickness=1),
        Line(points={{-40,-80},{8,0}},
          color={0,0,127},
          visible=datSou == AixLib.BoundaryConditions.Types.DataSource.Input,
          thickness=1),
