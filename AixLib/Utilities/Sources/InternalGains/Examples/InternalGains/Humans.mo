...
model Humans "Simulation to check the human models"
  ...
  AixLib.Utilities.Sources.InternalGains.Humans.HumanSensibleHeatTemperatureDependent
    humanSensibleHeat(RadiationConvertor(use_A_in=true), RatioConvectiveHeat=
        0.6) annotation (Placement(transformation(extent={{-24,-20},{22,32}})));
...
equation
  ...
  humanSensibleHeat.productHeatOutput.y = HeatOut;
  connect(varTempRoom.port, humanSensibleHeat.TRoom) annotation (Line(points={{
          -84,52},{-44,52},{-44,29.4},{-21.7,29.4}}, color={191,0,0}));
  connect(sine.y, varTempRoom.T) annotation(Line(points = {{-69.4, 24}, {-54, 24}, {-54, 52}, {-62, 52}}, color = {0, 0, 127}));
  connect(humanSensibleHeat.ConvHeat, fixedTemp.port) annotation (Line(points={
          {19.7,19},{38.85,19},{38.85,14},{58,14}}, color={191,0,0}));
  connect(humanSensibleHeat.RadHeat, fixedTemp.port) annotation (Line(
