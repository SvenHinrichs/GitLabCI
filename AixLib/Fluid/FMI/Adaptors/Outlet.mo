...
model Outlet "Adaptor for connecting a fluid outlet to the FMI interface"
  ...
  AixLib.Fluid.FMI.Interfaces.Outlet outlet(
...
equation
  ...
  if not allowFlowReversal then
    ...
    if Medium.nXi > 0 then
      // This test for nXi is needed for AixLib.Fluid.FMI.Validation.HeaterFan_noReverseFlow
      // to work with AixLib.Media.Water
