...
model Pressure "Test model for the pressure sensor"
  ...
  AixLib.Fluid.Sources.MassFlowSource_T masFloRat(
    redeclare package Medium = Medium,
    use_T_in=false,
    X={0.02,0.98},
    use_m_flow_in=true,
  ...
  AixLib.Fluid.Sensors.Pressure senPre_a(redeclare package Medium = Medium)
    "Pressure sensor at resistance port a"
    annotation (Placement(transformation(extent={{-22,20},{-2,40}})));
  AixLib.Fluid.Sensors.Pressure senPre_b(redeclare package Medium = Medium)
    "Pressure sensor at resistance port b"
    annotation (Placement(transformation(extent={{20,20},{40,40}})));
  AixLib.Fluid.Sensors.RelativePressure senRelPre(
